...
#define FG_FLASH 8
#define CL_FLASH 9
static int  fg_flash_init = 0;
static int  cl_flash_init = 0;
...
static int spi_geni_prepare_transfer_hardware(struct spi_master *spi)
{
	...
	fg_flash = of_property_read_bool(mas->dev->of_node,
							"qcom,fgflash");
	if(fg_flash&&fg_flash_init == 0){		
    		ret = gpio_request(FG_FLASH,"FG_FLASH");
    		dev_err(mas->dev,"ZD gpio_request FG_FLASH set to high gpio_request ret:%d \n" , ret);
		gpio_direction_output(FG_FLASH,1);
    		//gpio_free(FG_FLASH);
		fg_flash_init = 1;
		dev_err(mas->dev, "ZD found fgflash!\n");
	}

	cl_flash = of_property_read_bool(mas->dev->of_node,
							"qcom,crosslink");
	if(cl_flash&&cl_flash_init == 0){		
		ret = gpio_request(CL_RST,"CL_RST");
    		dev_err(mas->dev,"ZD gpio_request CL_RST set to low gpio_request ret:%d \n" , ret);
		gpio_direction_output(CL_RST,0);
    		//gpio_free(CL_RST);
    		ret = gpio_request(CL_FLASH,"CL_FLASH");
    		dev_err(mas->dev,"ZD gpio_request CL_FLASH set to high gpio_request ret:%d \n" , ret);
		gpio_direction_output(CL_FLASH,1);
    		//gpio_free(CL_FLASH);
    		cl_flash_init = 1;
		dev_err(mas->dev, "ZD found crosslink!\n");
	}
...
static int spi_geni_probe(struct platform_device *pdev)
{
	...
	fg_flash = of_property_read_bool(pdev->dev.of_node,
							"qcom,fgflash");
	if(fg_flash){
		
		rsc->geni_gpio_init = pinctrl_lookup_state(rsc->geni_pinctrl,
							"fgflashpin");
		if (IS_ERR_OR_NULL(rsc->geni_gpio_init)) {
		dev_err(&pdev->dev, "No init config specified!\n");
		ret = PTR_ERR(rsc->geni_gpio_init);
		goto spi_geni_probe_err;
		}
		retval = pinctrl_select_state(rsc->geni_pinctrl, rsc->geni_gpio_init);
	
		dev_err(&pdev->dev, "ZD pinctrl can select_state !\n");
		if (retval) {
			dev_err(&pdev->dev, "ZD spi pinctrl cannot select_state\n" );
		}
	}

	cl_flash = of_property_read_bool(pdev->dev.of_node,
							"qcom,crosslink");
	if(cl_flash){
		spi_gpio_vdd= regulator_get(&pdev->dev, "spi-gpio");
		if (IS_ERR(spi_gpio_vdd)) {
			dev_err(&pdev->dev, "ZD cannot get spi_gpio_vdd\n");					
		}else{
			dev_err(&pdev->dev, "ZD can get spi_gpio_vdd\n");
		}
		
		ret = regulator_enable(spi_gpio_vdd);
		if (ret) {
			dev_err(&pdev->dev, "ZD unable to enable spi_gpio_vdd\n");
		}else{
			dev_err(&pdev->dev, "ZD enable to enable spi_gpio_vdd\n");
		}
